...
parameters:
    -
        ...
        ranges:
content_markdown: >-
    When the user's client Websocket connected to the AZ Websocket server, the server will periodically send a ping message to user (currently set as 5 seconds) and include a timestamp. After receiving this prompt message, the user should promptly return the pong message and include the same timestamp in the following format:
